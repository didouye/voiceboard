...
export class SetupWizardService {
  ...
  async checkVbCable(): Promise<boolean> {
    this.debugConsole.log(
      "info",
      "[SetupWizard] Checking VB-Cable installation...",
    );
    ...
    try {
      ...
      this.debugConsole.log(
        "info",
        `[SetupWizard] VB-Cable check result: installed=${status.installed}, device=${status.device_name || "none"}`,
      );

      if (status.installed) {
        this.debugConsole.log(
          "info",
          `[SetupWizard] VB-Cable found: ${status.device_name}`,
        );
        ...
        this.debugConsole.log(
          "warn",
          "[SetupWizard] VB-Cable NOT installed - wizard should appear",
        );
      ...
      this.debugConsole.log(
        "error",
        `[SetupWizard] Failed to check VB-Cable: ${errorMessage}`,
      );
