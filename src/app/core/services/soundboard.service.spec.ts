...
describe("SoundboardService", () => {
  ...
  beforeEach(() => {
    ...
    tauriSpy.loadSettings.and.returnValue(
      Promise.resolve({
    ...
    const fuzzySearchSpy = jasmine.createSpyObj("FuzzySearchService", [
      "search",
    ]);
    // Default: return results matching query in name
    fuzzySearchSpy.search.and.callFake(
      (query: string, sounds: Sound[]): SearchResult[] => {
        const normalizedQuery = query.toLowerCase();
        return sounds
          .filter((s) =>
            (s.customName || s.name).toLowerCase().includes(normalizedQuery),
          )
          .map((s) => {
    ...
    tauriServiceSpy = TestBed.inject(
      TauriService,
    ) as jasmine.SpyObj<TauriService>;
    fuzzySearchServiceSpy = TestBed.inject(
      FuzzySearchService,
    ) as jasmine.SpyObj<FuzzySearchService>;
  });
  ...
  describe("pads with search", () => {
    ...
    it("should sort pads by search score", () => {
      ...
      expect(
        padsWithSounds.every((p) => p.sound!.name.includes("barb")),
      ).toBeTrue();
    });
    ...
    it("should include matchedIndices in each pad", () => {
      ...
      expect(
        padsWithSounds.every((p) => Array.isArray(p.matchedIndices)),
      ).toBeTrue();
      // At least one should have non-empty matchedIndices (exact matches)
      expect(
        padsWithSounds.some((p) => p.matchedIndices.length > 0),
      ).toBeTrue();
    });
