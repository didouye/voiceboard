...
export class ImageSearchService {
  ...
  private async getVqdToken(query: string): Promise<string> {
    ...
    this.debug.log("debug", "[ImageSearch] vqd token response", {
      status: response.status,
      ok: response.ok,
    });

    if (!response.ok) {
      this.debug.log("error", "[ImageSearch] Failed to get vqd token", {
        status: response.status,
      });
      throw new Error(
        `Failed to get search token (status: ${response.status})`,
      );
    }

    const html = await response.text();
    this.debug.log("debug", "[ImageSearch] HTML received", {
      length: html.length,
    });
    ...
    if (!vqdMatch) {
      this.debug.log(
        "error",
        "[ImageSearch] Could not extract vqd token from HTML",
      );
      throw new Error("Could not extract search token");
    }

    this.debug.log("debug", "[ImageSearch] vqd token extracted", {
      token: vqdMatch[1].substring(0, 20) + "...",
    });
    return vqdMatch[1];
  }
  ...
  async search(
    ...
    this.debug.log("info", "[ImageSearch] Starting search", {
      query,
      page,
      perPage,
    });
    ...
    try {
      ...
      this.debug.log("debug", "[ImageSearch] Images response", {
        status: response.status,
        ok: response.ok,
      });

      if (!response.ok) {
        this.debug.log("error", "[ImageSearch] Search request failed", {
          status: response.status,
        });
      ...
      this.debug.log("debug", "[ImageSearch] Results received", {
      ...
      this.debug.log("info", "[ImageSearch] Search completed", {
  ...
  async downloadImage(
    ...
    this.debug.log("info", "[ImageSearch] Downloading image", {
    ...
    this.debug.log("debug", "[ImageSearch] Download response", {
      status: response.status,
      ok: response.ok,
    });

    if (!response.ok) {
      this.debug.log("error", "[ImageSearch] Failed to download image", {
        status: response.status,
      });
    ...
    this.debug.log("info", "[ImageSearch] Image downloaded", {
      extension,
