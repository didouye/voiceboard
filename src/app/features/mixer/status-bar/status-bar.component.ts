...
@Component({
  ...
  template: `
    <div class="h-14 bg-background border-t border-border flex">
      <!-- Input -->
      <div
        class="flex-1 px-4 py-2 flex flex-col justify-center border-r border-border"
      >
        <div class="flex items-center gap-2 mb-1">
          ...
          <span class="flex items-center gap-1">
            ...
            <span class="text-[10px] text-text-muted">{{
              mixer.isRunning() ? "Recording" : "Ready"
            }}</span>
          </span>
        </div>
        ...
        <div class="flex items-center gap-2 mb-1">
          ...
          <span class="flex items-center gap-1">
            <span
              class="w-2 h-2 rounded-full"
              [class]="
                mixer.isRunning() ? 'bg-accent animate-pulse' : 'bg-text-muted'
              "
            ></span>
            <span class="text-[10px] text-text-muted">{{
              mixer.isRunning() ? "Streaming" : "Ready"
            }}</span>
          </span>
        </div>
      ...
      <div class="flex-1 px-4 py-2 flex flex-col justify-center">
        <div class="flex items-center gap-2 mb-1">
          ...
          <span class="flex items-center gap-1">
            ...
            ></span>
            <span class="text-[10px] text-text-muted">{{
              previewLevel() > 0 ? "Playing" : "Monitor"
            }}</span>
          </span>
        </div>
...
export class StatusBarComponent implements OnInit, OnDestroy {
  ...
  readonly previewLevel = computed(() =>
    Math.max(this.previewEngineLevel(), this.monitoringLevel()),
  );
  ...
  readonly inputDeviceName = computed(() => {
    ...
    return (
      devices.find((d) => d.id === settings.audio.inputDeviceId)?.name ||
      "Unknown"
    );
  });
  ...
  readonly outputDeviceName = computed(() => {
    ...
    return (
      devices.find((d) => d.id === settings.audio.outputDeviceId)?.name ||
      "Unknown"
    );
  });
  ...
  readonly previewDeviceName = computed(() => {
    ...
    return (
      devices.find((d) => d.id === settings.audio.previewDeviceId)?.name ||
      "Custom"
    );
  });
  ...
  async ngOnInit(): Promise<void> {
    // Load device info
    const [settings, inputDevices, outputDevices, previewDevices] =
      await Promise.all([
    ...
    this.unlistenAudioLevels = await listen<AudioLevels>(
      "audio-levels",
      (event) => {
    ...
    this.unlistenPreviewLevel = await listen<number>(
      "preview-level",
      (event) => {
